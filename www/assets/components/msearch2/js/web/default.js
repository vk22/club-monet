var mSearch2 = {
    ...
    load: function (params, callback, append) {
        ...
        /////// появления номинала при выборе одной страны

            var hashFilters = mSearch2.Hash.get();

            if (jQuery.isEmptyObject(hashFilters) == true) {
            // console.log('старт / нет запроса');
            // console.log('data country: '+hashFilters['ms|made_in']);
            $('.cat-filter .nominal input').prop( "checked", false );
            $('.cat-filter .nominal').hide();
            $(".made_in input[type=checkbox]:not(:checked)").parents('.checkboxWrap').fadeIn();  
            } else {
            
            if (hashFilters['ms|made_in'].length > 0) {
                setTimeout( function() {
                    $('.cat-filter .nominal').show()
                }, 2000);
                $(".made_in input[type=checkbox]:not(:checked)").parents('.checkboxWrap').fadeOut();
            } else {
                $('.cat-filter .nominal input').prop( "checked", false );
                $('.cat-filter .nominal').hide();
                $(".made_in input[type=checkbox]:not(:checked)").parents('.checkboxWrap').fadeIn();                    
