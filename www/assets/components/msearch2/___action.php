...
switch ($action) {
    case 'filter':
        ...
        if (!empty($ids)) {
            ...
            $totalDur;
            //$modx->log(xPDO::LOG_LEVEL_ERROR,'$ids: '.print_r($ids,1));
            //// выбираем время из трека
            foreach ($ids as $id) {
               $result = $pdoFetch->getArray('msProduct',
                // Условие where
                array(
                    'id' => $id,
                ),
                ...
                array(
                    ...
                    'select' => array('Data' => 'article'),
                    'sortby' => 'Data.article',
                    'sortdir' => 'desc',
                )
            ); 
            if (!empty($result)) {
                $totalDur += $result['article'];
                $totalDurFin = gmdate("d H:i:s", $totalDur);
            }
            }
            $modx->log(xPDO::LOG_LEVEL_ERROR,'totalDur: '.$totalDurFin);


            //$totalDur = gmdate("i:s", $totalDur);
            //$modx->log(xPDO::LOG_LEVEL_ERROR,'totalDur: '.$totalDur);
        ...
        //$modx->log(modX::LOG_LEVEL_ERROR, $results,1);
       // $modx->log(xPDO::LOG_LEVEL_ERROR,'results: '.$results);
        ...
        $response = array(
            ...
            'data' => array(
                ...
                'total' => empty($totalDurFin) ? 0 : $totalDurFin,
                'page' => $page,
                'pages' => $pages,
