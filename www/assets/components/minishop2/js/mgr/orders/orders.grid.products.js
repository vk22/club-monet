miniShop2.grid.OrderProducts = function (config) {
    ...
    Ext.applyIf(config, {
        ...
        cls: 'minishop2-grid minishop2-grid-order-products',
        multi_select: false,
        stateful: true,
        stateId: config.id,
        ...
        var fields = {
            ...
            name: {
                ...
                renderer: function (value, metaData, record) {
                    metaData.attr = 'style="-webkit-user-select:text;-moz-user-select:text;user-select:text;"';

                    if (!value || !record['data']['product_id']) {
                        return value || '';
                    }

                    return String.format(
                        '<a href="index.php?a=resource/update&id={0}" class="ms2-link" target="_self" ' +
                        'style="-webkit-user-select:text;-moz-user-select:text;user-select:text;" ' +
                        'onmousedown="event.cancelBubble=true;" ondragstart="return false;">{1}</a>',
