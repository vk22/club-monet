...
class mspYooKassaPaymentAcceptProcessor extends mspYooKassaResponseProcessor
{
    ...
    public function process()
    {
        ...
        $response = null;
        if (method_exists($handler, 'confirmPayment')) {
            if ($payment = $handler->confirmPayment($Order)) {
                $response = $handler->changeOrderStatus($Order, 2);
            }
        }
...
return 'mspYooKassaPaymentAcceptProcessor';