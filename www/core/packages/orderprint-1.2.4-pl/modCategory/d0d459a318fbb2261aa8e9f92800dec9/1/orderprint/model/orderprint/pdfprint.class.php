...
class PdfPrint {
    ...
    public function __construct(modX &$modx, $orderid, $docid) {
        ...
        switch ($this->storeType) {
            case 'shk':
                ...
                break;
            case 'shk3':
                $this->modx->addPackage('shopkeeper3', $modx->getOption('core_path') . 'components/shopkeeper3/model/');
                break;
            case 'ms2':
                break;
        }
    ...
    public function setPlaceholders() {
        ...
        switch ($this->storeType) {
            case 'shk':
                ...
                break;
            case 'shk3':
                $orderData = $this->getOrderDataSHK3();
                break;
            case 'ms2':
    ...
    public function getOrderDataSHK3() {
        $orderData = array();
        $this->modx->addPackage('shopkeeper3', $this->modx->getOption('core_path') . 'components/shopkeeper3/model/');
        $q = $this->modx->newQuery('modUser');
        $q->where(array('id' => $this->order->get('userid')));
        $q->innerJoin('modUserProfile', 'Profile', 'Profile.internalKey = modUser.id');
        $q->select('modUser.username');
        ...
        if ($q->prepare() && $q->stmt->execute()) {
            ...
            if ($statuses = json_decode($statuses[0]['value'], true)){
                foreach ($statuses as $status_arr) {
                    if ($status_arr['id'] != $this->order->get('status')) {
                        continue;
                    }
                    $orderData['status'] = $status_arr['label'];
                    break;
                }
            }
        }
        ...
        if (!empty($contacts)) {
            foreach ($contacts as $index => $elem) {
                $orderData['contacts.' . $elem['name']] = $elem['value'];
            }
        }
        $i = 1;
        if ($tpl = $this->document->get('chunk2')) {
            $cart = '';
            foreach ($this->order->shk_purchases as $product) {
                ...
                $productArray = array();
                $productArray['idx'] = $i;
                $productArray['id'] = $product['p_id'];
                $productArray['count'] = $product['count'];
                $productArray['price'] = $product['price'];
                $productArray['cost'] = $product['count'] * $product['price'];
                $productArray['name'] = $product['name'];
                if (!empty($product['data'])) {
                    $data = json_decode($product['data'], 1);
                    foreach ($data as $dataKey => $dataValue) {
                        if ($dataKey == 'url') {
                            $productArray['data.link'] = $dataValue;
                        }
                        $productArray['data.' . $dataKey] = $dataValue;
                    }
                }
                
                if (!empty($product['options'])) {
                    $data = json_decode($product['options'], 1);
                    foreach ($data as $dataKey => $dataValue) {
                        if (is_array($dataValue)) {
                            foreach ($dataValue as $index => $elem) {
                                $productArray['options.' . $dataKey . '.' . $index] = $elem;
                            }
                        } else {
                            $productArray['options.' . $dataKey] = $dataValue;
                        }
                    }
                }
                $cart .= $this->getChunk($tpl, $productArray);
		...
		$orderData['cart_count'] = count($this->order->shk_purchases);
        $orderData['date'] = date('d.m.Y', strtotime($order["date"]));
        $orderData['createdon_raw'] = strtotime($order["date"]);
        $orderData['sentdate'] = date('d.m.Y', strtotime($order["sentdate"]));
        unset($order["contacts"], $order["content"], $order["userid"], $order["addit"], $order["status"], $order["date"], $order["sentdate"]);
        ...
        $orderData['manager'] = $this->user->Profile->get('fullname');
        if (!isset($orderData['delivery']) || !$orderData['delivery']) {
            $conf = $this->modx->getObject('shk_config', array('setting' => 'delivery'));
            $conf = unserialize($conf->value);
            $orderData['delivery'] = $conf[0]['label'];
        }
    ...
    public function getOrderDataMS2() {
        ...
        if($this->order->Delivery){
            ...
            $orderData['delivery'] = '';
            $orderData['payment_description'] = '';
            $orderData['payment_price'] = '';
        }
		...
		$orderData['total_count'] = 0;
            
        if ($tpl = $this->document->get('chunk2')) {
            $options = array();
            $all_options = $this->modx->getCollection('msOption');
            ...
            foreach ($this->order->Products as $product) {
                ...
                foreach($product->toArray() as $key => $value){
                    $productArray[$key] = $product->get($key);
                }
                $orderData['total_count'] = $orderData['total_count'] + $product->get('count');
                // $productArray['id'] = $product->get('id');
                // $productArray['name'] = $product->get('name');
                ...
                if(isset($product->Product) && gettype($product->Product) == 'object'){
                    foreach($product->Product->toArray() as $key => $value){
                        $productArray['original.'.$key] = $product->Product->get($key);
                    }
                    foreach($product->Product->Data->toArray() as $key => $value){
                        $productArray['original.'.$key] = $product->Product->Data->get($key);
                    }
                ...
                if(!empty($productOptions)){
                    ...
                    if(isset($product->Product) && gettype($product->Product) == 'object'){
                        $productOptions = $product->Product->Data->get('options');
                        if(!empty($productOptions)){
                            foreach ($productOptions as $option => $value) {
                                $productArray['options.' . $option] = $value[0];
                            }
                        }
                    }
                }
    ...
    public function getChunk($tpl, $pls){
		...
		if ($this->modx->loadClass('pdofetch', MODX_CORE_PATH . 'components/pdotools/model/pdotools/', false, true)) {
            ...
            return $pdoFetch->getChunk($tpl, $pls);
        }
