...
if (!class_exists('msPaymentInterface')) {
    if (file_exists(MODX_CORE_PATH . 'components/minishop2/handlers/mspaymenthandler.class.php')) {
...
class Sberbank extends msPaymentHandler implements msPaymentInterface
{
    ...
    function __construct(xPDOObject $object, $config = array())
    {
        ...
        $this->config = array_merge(array(
            ...
            'apiDopUrl' => $apiDopUrl,
            'debug' => $this->modx->getOption('ms2_payment_sbrbnk_debug', null, false, true),
        ) , $config);
    ...
    public function getPaymentLink(msOrder $order)
    {
        ...
        if (!$paymentLink = $this->modx->getObject('mspSberbankLink', array('order' => $id))) {
            ...
            if ($order->user_id) {
                ...
                if ($user && $this->modx->getOption('ms2_payment_sbrbnk_send_customer', null, 0)) {
            ...
            $request['orderBundle'] = str_replace('"customerDetails":[]', '"customerDetails":{}', $request['orderBundle']);
            ...
            if ($this->config['debug']) {
                $log[] = 'REQUEST: ' . PHP_EOL . print_r($request, true);
            }
            ...
            if ($this->config['debug']) {
                $log[] = 'RESPONSE: ' . PHP_EOL . print_r($respond, true);
            }
            if (!empty($respond)) {
        ...
        if (!empty($log)) {
            $this->modx->log(modX::LOG_LEVEL_ERROR, '[miniShop2:Sberbank] ' . implode(PHP_EOL, $log));
        }
